<template>
  <v-container>
    <v-card>
      <v-card-title>
        Approved Student: {{ studentInfo?.username }} (Request #{{ requestId }})
      </v-card-title>
      <v-card-text>
        <div v-if="studentInfo">
          <strong>Student Name:</strong> {{ studentInfo.username }}<br>
    ...
    <v-row class="mt-4">
      <v-col cols="12"
        ...
        <v-card class="file-section-card flex-grow-1 d-flex flex-column justify-space-between">
          <!-- Colored strip for section title -->
...
<script setup lang="ts">
  ...
  const fetchRequestDetails = async () => {
    const reqRes = await axios.get(`/api/registrationrequest/${requestId}`);
    studentInfo.value = {
      username: reqRes.data.student?.user?.username,
      studentNumber: reqRes.data.student?.studentNumber,
      email: reqRes.data.student?.user?.email
    };
    const filesRes = await axios.get(`/api/fileupload/by-request/${requestId}`);
  ...
  const uploadNewFile = async (type: string, file: File) => {
    ...
    formData.append('uploadedBy', 'professor'); // or get from userStore
